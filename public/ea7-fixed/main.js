var app = (() => {
	...
	function initPipline() {
		...
		// Provide viewport size to fragment shader (if uniform exists)
		if (prog.viewportSizeUniform) gl.uniform2f(prog.viewportSizeUniform, gl.viewportWidth, gl.viewportHeight);
	}
	...
	function initUniforms() {
		...
		// Viewport size for fragment shader (used with gl_FragCoord)
		prog.viewportSizeUniform = gl.getUniformLocation(prog, "uViewportSize");
		// Depth reconstruction uniforms
		prog.nearUniform = gl.getUniformLocation(prog, "uNear");
		prog.farUniform = gl.getUniformLocation(prog, "uFar");
		prog.depthRangeUniform = gl.getUniformLocation(prog, "uDepthRange");
	}
	...
	function initLighting() {
		...
		// Slightly darker fog so distant areas don't clamp to full white
		gl.uniform3fv(prog.fogColorUniform, new Float32Array([0.75, 0.82, 0.88]));
	...
	function initModels() {
		...
		const floorModel = new Model(
			new Plane({ width: 64.0, height: 64.0, segmentsX: 16, segmentsZ: 16 }),
			gl, prog,
			{
				fillstyle: 'fill',
				color: [0.5, 0.7, 0.4],
	...
	function updateFpsCamera(dt) {
		...
		// Move with WASD: A/D = horizontal (XZ), W/S = vertical (Y)
		let move = [0, 0, 0];
		// W = up, S = down
		if (_keys.has('w')) { move[1] += 1; }
		if (_keys.has('s')) { move[1] -= 1; }
		// A/D remain horizontal relative to camera yaw
		if (_keys.has('a')) { move[0] -= right[0]; move[2] -= right[2]; }
		if (_keys.has('d')) { move[0] += right[0]; move[2] += right[2]; }
		// Normalize move across all axes so diagonal movement isn't faster
		const mLen = Math.sqrt(move[0] * move[0] + move[1] * move[1] + move[2] * move[2]);
		if (mLen > 0.0001) { move[0] /= mLen; move[1] /= mLen; move[2] /= mLen; }
		// Apply movement
		camera.eye[0] += move[0] * camera.moveSpeed * dt;
		camera.eye[1] += move[1] * camera.moveSpeed * dt;
	...
	function setProjection() {
		...
		// Update viewport-related uniforms used by fragment shader
		if (prog.viewportSizeUniform) gl.uniform2f(prog.viewportSizeUniform, gl.viewportWidth, gl.viewportHeight);

		// Set near/far and depth-range used for linear depth reconstruction
		let nearVal = 1.0;
		let farVal = 170.0;

		if (prog.nearUniform) gl.uniform1f(prog.nearUniform, nearVal);
		if (prog.farUniform) gl.uniform1f(prog.farUniform, farVal);
		// Depth range controls where brightness mapping starts/ends (world units)
		const depthStart = 1.0;
		const depthEnd = 170.0;
		if (prog.depthRangeUniform) gl.uniform2f(prog.depthRangeUniform, depthStart, depthEnd);
	}
